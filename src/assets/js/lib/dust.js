...
class Dust {
  constructor(renderingEngine = PIXI) {
    if (renderingEngine === undefined)
      throw new Error(
        "Please assign a rendering engine in the constructor before using pixiDust.js"
      );
  ...
  create(
    ...
    let makeParticle = (angle) => {
      ...
      particle.rotationSpeed = this.randomFloat(
        minRotationSpeed,
        maxRotationSpeed
      );
    ...
    if (this.globalParticles.length > 0) {
      ...
      for (let i = this.globalParticles.length - 1; i >= 0; i--) {
        ...
        //contain any more sprites
          ...
          this.globalParticles.splice(
            this.globalParticles.indexOf(particles),
            1
          );
        }
      }
